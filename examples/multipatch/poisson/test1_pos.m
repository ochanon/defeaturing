...
filename = 'results/test1_drchlt0';
saveIt = false;
plotIt = true;
...
if plotIt
    fig = figure;
    loglog(eps_values, errh1s, '+-r', eps_values, est, '+-b', eps_values, eps_values*0.3, 'k:');
    grid on
    ...
    loglog(eps_values, errh1s, '+-r', eps_values, errh1s_0, '+-g', ...
        eps_values, errh1s_F, '+-c', eps_values, est, '+-b', eps_values, eps_values*0.3, 'k:');
    grid on
...
function [srf_0, srf, srf_F] = buildGeometry(epsilon, ext)
    ...
    srf(3) = nrbkntins(srf(3), {1-(1-ext*epsilon)/(1-epsilon), 1-(ext-1)*epsilon/2});
    
    srf(4) = nrb4surf ([0.5-epsilon/2 1], [0.5+epsilon/2 1], [0.5-epsilon/2 1+epsilon], [0.5+epsilon/2 1+epsilon]);
