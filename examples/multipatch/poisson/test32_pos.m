...
filename = 'results/test32_drchlt0';
saveIt = false;
plotIt = true;
...
if plotIt
    fig = figure;
    loglog(eps_values, errh1s, '+-r', eps_values, est, '+-b', eps_values, eps_values.^(5/2)*0.7, 'k:');
    grid on
    ...
    loglog(eps_values, errh1s, '+-r', eps_values, errh1s_0, '+-g', ...
        eps_values, errh1s_F, '+-c', eps_values, est, '+-b', eps_values, eps_values.^(5/2)*0.7, 'k:');
    grid on
...
function [srf_0, srf, srf_F] = buildGeometry(epsilon, ext)    
    ...
    srf(1) = nrbkntins(srf(1), {(1-ext*epsilon)/(1-epsilon), 1-ext*epsilon, []});
    
    srf(2) = nrb4surf ([1-epsilon 0], [1 0], [1-epsilon 1], [1 1]);
    srf(2) = nrbextrude(srf(2), [0, 0, epsilon]);
    srf(2) = nrbkntins(srf(2), {0.5, 1-ext*epsilon, []});
    
    srf(3:4) = srf(1:2);
    
    for ii = 3:4
        srf(ii).coefs(3,:,:,:) = srf(ii).coefs(3,:,:,:)*(1/epsilon-1)+epsilon;
        srf(ii) = nrbkntins(srf(ii), {[], [], 1-(1-ext*epsilon)/(1-epsilon)});
    end
